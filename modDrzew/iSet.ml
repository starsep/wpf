...
type range = { left: int; right: int; below: int }
type set =
  | Empty
  | Node of set * range * set * int
...
let cmp { left = a; right = b; below = _ }
		{ left = c; right = d; below = _ } =
		if b < c then -1
...
let iter f { cmp = _; set = set } =
  let rec loop = function
    | Empty -> ()
    | Node (l, k, r, _) -> loop l; f k; loop r in
  loop set

let fold f { cmp = _; set = set } acc =
